...
def test_kill_records_the_co_present_set(monkeypatch) -> None:
    ...
    b = _belief()
    # blue+cyan are with us throughout; green (victim) last seen alive at t=5; red joins at t=6.
    for t in range(1, 6):
    ...
    sets = alibi_sets(b)
    assert sets == [frozenset({"blue", "cyan"})]  # red joined after t=5, green is the victim
